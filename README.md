...
2. Inspect the gold **REPLAY MODE** trace. Its provenance banner states that GPT-5.6 Sol generated the actions inside Codex—not through the OpenAI API—and the checked-in source action file is reproducibly adjudicated by sonuv-guard.
3. If the server later has usable API quota, choose **Run live audit**. Otherwise the failed live request automatically returns to the same labeled replay.
...
The gate runs 36 backend tests, including exact reproduction of the Codex-generated Guard trace, frontend interaction tests, TypeScript checking, a production build, vendored-source hashes, and—inside the original source workspace—the upstream 121-test sonuv-guard regression suite.
