...
- 不复用旧游戏的全局 session。
- 复用现有 `sonuv-guard` Python 包，但不修改其源码。
...
Vercel 只托管静态前端。Railway 保存 OpenAI API key、通过 OpenAI Python SDK 的异步 Responses API 调用 GPT-5.6，并执行动作校验、Guard 裁决、限流和会话管理。Docker 从 `game` 仓库根目录构建，安装现有 `sonuv-guard` 包并复制 `hackathon/backend`。
